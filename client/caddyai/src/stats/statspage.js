...
const StatsPage = ({ email }) => {
  ...
  const fetchParValues = async (courseName) => {
    try {
      const response = await axios.get(
        "http://localhost:3001/courses/coursesearch",
        {
      ...
      const parValues = [
        response.data.hole1_par,
        response.data.hole2_par,
        response.data.hole3_par,
        response.data.hole4_par,
        response.data.hole5_par,
        response.data.hole6_par,
        response.data.hole7_par,
        response.data.hole8_par,
        response.data.hole9_par,
        response.data.hole10_par,
        response.data.hole11_par,
        response.data.hole12_par,
        response.data.hole13_par,
        response.data.hole14_par,
        response.data.hole15_par,
        response.data.hole16_par,
        response.data.hole17_par,
        response.data.hole18_par,
      ];
    ...
    const fetchUserData = async () => {
      try {
        ...
        const statsResponse = await axios.get(
          "http://localhost:3001/stats/stats",
          {
            params: { email },
          }
        );
        ...
        const recentGameResponse = await axios.get(
          "http://localhost:3001/stats/recent-game",
          {
            params: { email },
          }
        );
  ...
  const displayedStats =
    viewMode === "latest" && recentGame ? recentGame : statsData;
  const totalFairways = displayedStats.length;

  const fairwayHits = displayedStats.filter((post) => post.fairwayHit);
  const fairwaysHitPercentage = (
    (fairwayHits.length / totalFairways) *
    100
  ).toFixed(2);

  const greensHits = displayedStats.filter((post) => post.gir);
  const greensHitPercentage = (
    (greensHits.length / totalFairways) *
    100
  ).toFixed(2);
        ...
        <Container className="mt-4">
          ...
          <Row>
            <Col>
              ...
              <h2>
                {(
                  statsData.reduce((sum, post) => sum + post.putts, 0) /
                  totalFairways
                ).toFixed(2)}
              </h2>
            </Col>
