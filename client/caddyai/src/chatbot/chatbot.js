...
import ShotFeedBackForm from "./ShotFeedbackForm.js";
...
const Chatbot = ({ email, encodedShotData }) => {
  ...
  const clubSelectionAI = useCallback(
    async (prompt) => {
      try {
  ...
  const handleCaddyFormSubmit = useCallback(
    async (data) => {
      setCaddyFormData(data);
      var finalPrompt = parseCaddyShotData(data);
      const encodedShotDataString = JSON.stringify(encodedShotData);
      ...
      const newMessage = {
        text: "What club to play for this shot?",
        type: "sender",
      };
      const updatedHistory = [...messageHistory, newMessage];
      setMessageHistory(updatedHistory);
    ...
    <div className="bottom-section">
      <div ref={chatContainerRef} className="chat-container">
        {messageHistory.map((msg, index) => (
          <ChatMessage
            ...
            msg={msg}
            index={index}
            handleOpenNewForm={handleOpenNewForm}
          />
        ))}
        {loading && <LoadingIcon />}
      </div>
      ...
      {showForm && (
        <ShotPredictionForm
          ...
          onSubmit={handleCaddyFormSubmit}
        />
      )}
      ...
      <button className="open-form-button" onClick={handleShowForm}>
        Caddy ⛳
      </button>
